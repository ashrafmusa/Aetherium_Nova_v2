...
export function executeContract(contractJavascriptCode, transaction, block, initialStorage, readOnly = false) {
    const logs = [];
    let events = [];
    ...
    const consumeGasHostFunction = (amount) => {
    ...
    const bridge = {
        ...
        blockchain: {
            ...
            getBalance: new ivm.Reference((address) => {
                consumeGasHostFunction(GENESIS_CONFIG.gasCosts?.STORAGE_READ ?? 10);
                return getBalance(address);
            }),
            transfer: new ivm.Reference((recipient, amount) => {
                if (readOnly) {
            ...
            callContract: new ivm.Reference((targetAddress, methodName, params = [], value = 0) => {
                if (readOnly) {
        ...
        emitEvent: new ivm.Reference((name, ...args) => {
            if (readOnly) {
        ...
        setReturnValue: new ivm.Reference((value) => {
            returnValue = value;
        })
    };
    ...
    try {
        ...
        const contractClassName = transaction.data?.contractClassName;
