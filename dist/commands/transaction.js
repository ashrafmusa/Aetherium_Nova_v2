...
export function registerTransactionCommands(program) {
    ...
    program
        ...
        if (!fs.existsSync(filepath)) {
            console.error(chalk.red(`❌ Contract source file not found at: ${filepath}`));
            return;
        }
        const feeValidation = validateAmountFee(fee, "Fee", GENESIS_CONFIG.minFee);
        if (feeValidation !== true) {
        ...
        const compiledContractPath = path.join(process.cwd(), 'dist', 'contracts', contractBaseName + '.js');
        let code;
        try {
    ...
    program
        ...
        if (contractValidation !== true) {
            console.error(chalk.red(`❌ ${contractValidation}`));
            return;
        }
        const feeValidation = validateAmountFee(fee, "Fee", GENESIS_CONFIG.minFee);
        if (feeValidation !== true) {
        ...
        if (!method || typeof method !== 'string') {
            console.error(chalk.red("❌ Method name is required."));
            return;
        }
        ...
        try {
            ...
            if (res) {
                ...
                if (res.logs && res.logs.length > 0) {
                    ...
                    res.logs.forEach((log) => console.log(chalk.gray(`  - ${JSON.stringify(log)}`)));
                }
            }
            else {
