...
import { getTransactionId, createRewardTransaction } from './Transaction.js';
import { clearLedger, processBlockTransactions, rebuildLedgerFromChain } from './ledger_processor.js';
import { saveLedgerToDisk, loadLedgerFromDisk, _getBalancesMap, _getNoncesMap, _getContractCodeMap, _getContractStorageMap, _setBalancesMap, _setNoncesMap, _setContractCodeMap, _setContractStorageMap } from './ledger.js';
import mempool from './pool.js';
import logger from './logger.js';
import { GENESIS_CONFIG } from './config.js';
import { addOrUpdateValidator, saveStakingLedgerToDisk, loadStakingLedgerFromDisk, chooseNextBlockProposer, getValidator, _getValidatorsMap, _setValidatorsMap, updateEpochValidatorSet } from './staking.js';
...
export function calculateBlockHash(block) {
    ...
    const txString = JSON.stringify([...data].sort((a, b) => {
...
export function getBlockchain() { return [...chain]; }
export function getLatestBlock() { return chain[chain.length - 1]; }
export async function proposeBlock(pendingTxs, proposerAddress, proposerPublicKey, proposerSignature, timestamp) {
    ...
    const newBlock = { ...blockData, hash, signature: proposerSignature };
...
export async function isValidChain(chainToValidate) {
    ...
    for (let i = 1; i < chainToValidate.length; i++) {
        ...
        if (currentBlock.proposer !== expectedProposer) {
            logger.error(`[Chain] Invalid chain: Proposer ${currentBlock.proposer.slice(0, 10)}... was not expected proposer ${expectedProposer?.slice(0, 10)}... for block ${currentBlock.index}`);
            return false;
        }
...
export async function replaceChain(newChain) {
