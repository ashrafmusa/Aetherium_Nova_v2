...
import { getBlockchain, replaceChain } from './chain.js';
import { getTransactionId } from './Transaction.js';
import { NETWORK_CONFIG } from './config.js';
import logger from './logger.js';
let peers = new Set(NETWORK_CONFIG.seedPeers);
...
export function getPeers() { return Array.from(peers); }
export async function addPeer(url, propagate = true) {
...
export async function connectToPeer(url) {
    await addPeer(url, true);
    await syncWithPeers();
}
async function broadcast(endpoint, data, originPeer) {
...
export const broadcastBlock = (block, originPeer) => broadcast('/block', block, originPeer);
export const broadcastTransaction = (tx, originPeer) => broadcast('/transaction', tx, originPeer);
export async function gossipPeers() {
    ...
    for (const peer of currentPeers) {
        try {
            ...
            if (peersResponse.data && Array.isArray(peersResponse.data.peers)) {
                const discoveredPeers = peersResponse.data.peers;
                for (const discoveredPeer of discoveredPeers) {
                    await addPeer(discoveredPeer, false);
                }
