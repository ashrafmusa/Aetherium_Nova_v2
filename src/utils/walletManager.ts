...
import { Wallet, decryptPrivateKey, encryptPrivateKey } from "../wallet.js";
...
export async function loadAndDecryptWallet(address: string): Promise<Wallet | null> {
  ...
  try {
    const walletData: Wallet = JSON.parse(fs.readFileSync(walletPath, "utf-8"));
    ...
    console.error(chalk.red(`❌ Error loading or parsing wallet file ${address}:`), (e as Error).message);
    return null;
  }
}
